...
def check_login():
    if not st.session_state["logged_in"]:
        ...
        with c2:
            ...
            st.markdown("<p style='text-align: center; color: gray;'>V 4.1 流式生成修复版</p>", unsafe_allow_html=True)
            with st.form("login"):
...
if st.session_state["logged_in"] and st.session_state["first_visit"]:
    ...
    st.markdown("<p style='text-align: center;'>功能全开 · 流式生成 · 交互修复</p><br>", unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    c1.info("🛠️ 侧边栏：支持自定义类型与基调"); c2.info("🗺️ 蓝图：支持一个字一个字流式生成"); c3.info("🤐 写作：支持分栏对照模式")
...
with tab_write:
    # 1. 备战区
    with st.expander("🎬 备战区：素材投喂", expanded=True):
    ...
    with c_d4: burst_mode = st.toggle("💥 注水模式", False)
    st.markdown("</div>", unsafe_allow_html=True)

    st.divider()
    use_split_view = st.toggle("📖 对照模式", value=True)
    ...
    with col_write:
        ...
        if user_input:
            ...
            if burst_mode: sys_p += "【强制要求】强力注水模式，大量细节。\n"
...
# --- TAB 2: 创世蓝图 (修复版：流式生成 + 交互反馈) ---
with tab_blueprint:
    st.markdown("### 🗺️ 创世蓝图 (Ideation)")
    st.info("✨ 已启用流式生成：不再卡顿，实时显示。输入框与结果框已分离。")
    ...
    # [Input] 初始输入框 (使用 text_area 增加输入空间)
    ...
    if generate_idea:
        ...
        st.session_state["bp_idea_res"] = "" # 清空旧结果
        ...
        with st.spinner("AI 正在构思..."):
            ...
            stream = client.chat.completions.create(
                model="deepseek-chat", 
                messages=[{"role":"system","content":planner_sys},{"role":"user","content":p}], 
                stream=True # 开启流式
            )
            # 实时打印流
            response = st.write_stream(stream)
            ...
            st.rerun() # 强制刷新以显示结果编辑框

    # [Result & Feedback] 只有生成过才有
    if st.session_state["bp_idea_res"]:
        st.markdown("---")
        # 结果编辑框
        st.session_state["bp_idea_res"] = st.text_area("✅ 脑洞结果 (可手动修改)", st.session_state["bp_idea_res"], height=150)
