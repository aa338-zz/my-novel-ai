...
def check_login():
    if not st.session_state["logged_in"]:
        ...
        with c2:
            ...
            st.markdown("<p style='text-align: center; color: gray;'>V 3.1 交互修正版</p>", unsafe_allow_html=True)
            with st.form("login"):
...
with st.sidebar:
    ...
    with st.container():
        ...
        # 核心基调 (修复：增加自定义选项)
        tone_opts = ["热血 / 王道 / 爽文", "暗黑 / 压抑 / 生存", "轻松 / 搞笑 / 吐槽", "悬疑 / 烧脑 / 反转", "治愈 / 情感 / 细腻", "【✏️ 自定义...】"]
...
# --- TAB 2: 创世蓝图 (完全重构版 - 修复交互逻辑) ---
with tab_blueprint:
    st.markdown("### 🗺️ 创世蓝图 (Genesis Blueprint)")
    st.info("交互逻辑已修复：输入框和结果框分离，支持反复生成，AI 废话已过滤。")
    
    # 定义强力约束 System Prompt
    bp_sys = (
        f"你是一个网文策划。类型：{st.session_state['global_genre']}。基调：{st.session_state['global_tone']}。\n"
        "【严禁废话】不要输出'好的'、'以下是...'、'需要我继续吗'等客套话。\n"
        "【严禁提问】不要在结尾询问用户意见。\n"
        "直接输出策划内容本身。"
    )
    ...
    # [Input] 使用独立的临时变量
    input_idea = st.text_area("✍️ 输入/修改你的原始点子", value=st.session_state.get("bp_input_cache", st.session_state["pipe_idea"]), height=100, key="bp_input_area")
    
    c_bp1, c_bp2 = st.columns([1, 4])
    if c_bp1.button("✨ 生成/重写脑洞"):
        st.session_state["bp_input_cache"] = input_idea # 记住用户的输入
        with st.spinner("AI 构思中..."):
            p = f"基于点子“{input_idea}”，完善成一个有吸引力的核心梗，增加冲突和期待感。字数200字内。"
    ...
    # [Output] 结果框，绑定 pipe_idea
    if st.session_state["pipe_idea"]:
        st.session_state["pipe_idea"] = st.text_area("✅ 脑洞结果 (AI 生成，可手动修改)", st.session_state["pipe_idea"], height=150)
    ...
    if st.button("📜 生成前三章细纲"):
        with st.spinner("推演剧情..."):
            p = (
                ...
                f"请生成前三章细纲。要求：每一章都有标题，列出关键事件和爽点。严禁客套话。"
            )
            r = client.chat.completions.create(model="deepseek-chat", messages=[{"role":"system","content":bp_sys},{"role":"user","content":p}])
            # 简单的后处理，防止 AI 即使被禁止了还在最后问问题
            clean_res = r.choices[0].message.content.replace("需要我为您继续构思吗？", "").replace("您觉得如何？", "")
            st.session_state["pipe_outline"] = clean_res
            st.rerun()
